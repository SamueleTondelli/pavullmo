...
MUON_LR_MULTIPLIER = float(os.environ.get("MUON_LR_MULTIPLIER", 1.0))
...
MUON_NS_COEFFICIENT_B = float(os.environ.get("MUON_NS_COEFFICIENT_B", -4.775))
MUON_NS_COEFFICIENT_C = float(os.environ.get("MUON_NS_COEFFICIENT_C", 2.0315))
...
def validate_configuration(train_dataset: TokenBlockDataset) -> None:
    ...
    if MUON_ADJUST_LR_FN not in {"original", "match_rms_adamw"}:
        raise ValueError(
            "MUON_ADJUST_LR_FN must be either 'original' or 'match_rms_adamw'"
        )
    if MUON_EPS <= 0.0:
        raise ValueError("MUON_EPS must be positive")
    if not 0 < MUON_NS_ITERS < 100:
        raise ValueError("MUON_NS_ITERS must be in [1, 99]")
    if not all(
        ...
        raise ValueError("Muon Newton-Schulz coefficients must be finite")
...
def is_muon_parameter(
    parameter_name: str,
    parameter: torch.nn.Parameter,
) -> bool:
    """Return whether a trainable parameter belongs to the Muon optimizer."""

    return (
        parameter.requires_grad
        and parameter.ndim == 2
        and parameter_name != "embeddings.weight"
    )
...
def build_adamw_parameter_groups(
    ...
    for name, parameter in model.named_parameters():
        if not parameter.requires_grad or is_muon_parameter(name, parameter):
            continue
...
def build_muon_parameter_groups(
    model: torch.nn.Module,
) -> list[dict[str, Any]]:
    """Build the Muon group for hidden two-dimensional weight matrices."""
    ...
    return [
        {"params": parameters, "weight_decay": MUON_WEIGHT_DECAY},
    ]
...
def duplicate_parameter_ids(parameter_id_list: list[int]) -> set[int]:
    seen: set[int] = set()
...
def validate_optimizer_parameter_partition(
    model: torch.nn.Module,
    adamw_parameter_groups: list[dict[str, Any]],
    muon_parameter_groups: list[dict[str, Any]],
) -> None:
    """Ensure every trainable parameter is assigned to exactly one optimizer."""
    ...
    adamw_ids = parameter_ids(adamw_parameter_groups)
    muon_ids = parameter_ids(muon_parameter_groups)

    duplicate_adamw_ids = duplicate_parameter_ids(adamw_ids)
    duplicate_muon_ids = duplicate_parameter_ids(muon_ids)
    overlap_ids = set(adamw_ids) & set(muon_ids)
    assigned_ids = set(adamw_ids) | set(muon_ids)
    missing_ids = expected_ids - assigned_ids
    unexpected_ids = assigned_ids - expected_ids
    if (
        duplicate_adamw_ids
        or duplicate_muon_ids
        or overlap_ids
        or missing_ids
        or unexpected_ids
    ):
        def names(parameter_ids_to_report: set[int]) -> list[str]:
            return sorted(
                ...
                for parameter_id in parameter_ids_to_report
            )

        raise RuntimeError(
            "invalid AdamW/Muon parameter partition: "
            f"duplicate_adamw={names(duplicate_adamw_ids)}, "
            f"duplicate_muon={names(duplicate_muon_ids)}, "
            f"overlap={names(overlap_ids)}, missing={names(missing_ids)}, "
            f"unexpected={names(unexpected_ids)}"
        )
...
def sampled_logit_stats(logits: torch.Tensor) -> tuple[float, float]:
    ...
    sample = logits.detach()[..., ::sequence_stride, ::vocabulary_stride].float()
...
def snapshot_parameters(
    ...
    """Copy parameters to CPU so diagnostics can measure the exact next update."""

    return {
        name: parameter.detach().to(device="cpu", dtype=torch.float32, copy=True)
        for name, parameter in named_parameters
    }


def empty_update_accumulator() -> dict[str, Any]:
    return {
        "parameter_sum_of_squares": 0.0,
        "post_parameter_sum_of_squares": 0.0,
        "update_sum_of_squares": 0.0,
        "parameter_update_dot": 0.0,
        "element_count": 0,
        "matrix_angles": [],
    }


def add_parameter_update_to_accumulator(
    ...
    *,
    ...
    parameter_sum_of_squares = before.square().sum(dtype=torch.float64).item()
    post_parameter_sum_of_squares = after.square().sum(dtype=torch.float64).item()
    update_sum_of_squares = update.square().sum(dtype=torch.float64).item()
    parameter_update_dot = (before * update).sum(dtype=torch.float64).item()

    accumulator["parameter_sum_of_squares"] += parameter_sum_of_squares
    accumulator["post_parameter_sum_of_squares"] += post_parameter_sum_of_squares
    accumulator["update_sum_of_squares"] += update_sum_of_squares
    accumulator["parameter_update_dot"] += parameter_update_dot
    accumulator["element_count"] += before.numel()

    if is_matrix and parameter_sum_of_squares > 0.0:
        parameter_norm = math.sqrt(parameter_sum_of_squares)
        radial_update = parameter_update_dot / parameter_norm
        tangential_update = math.sqrt(
            max(0.0, update_sum_of_squares - radial_update**2)
        )
        accumulator["matrix_angles"].append(
            math.atan2(tangential_update, parameter_norm + radial_update)
        )


def finalize_update_accumulator(accumulator: dict[str, Any]) -> dict[str, float]:
    element_count = int(accumulator["element_count"])
    if element_count == 0:
        return {
            "parameter_norm": 0.0,
            "post_parameter_norm": 0.0,
            "parameter_rms": 0.0,
            "post_parameter_rms": 0.0,
            "update_norm": 0.0,
            "update_rms": 0.0,
            "update_to_parameter_norm": 0.0,
            "update_weight_cosine": 0.0,
            "radial_update_fraction": 0.0,
            "angular_step": 0.0,
            "matrix_angular_step_mean": 0.0,
            "matrix_angular_step_max": 0.0,
        }

    parameter_norm = math.sqrt(accumulator["parameter_sum_of_squares"])
    post_parameter_norm = math.sqrt(accumulator["post_parameter_sum_of_squares"])
    update_norm = math.sqrt(accumulator["update_sum_of_squares"])
    parameter_update_dot = accumulator["parameter_update_dot"]
    denominator = parameter_norm * update_norm
    update_weight_cosine = (
        parameter_update_dot / denominator if denominator > 0.0 else 0.0
    )
    update_weight_cosine = min(max(update_weight_cosine, -1.0), 1.0)
    radial_update = (
        parameter_update_dot / parameter_norm if parameter_norm > 0.0 else 0.0
    )
    tangential_update = math.sqrt(max(0.0, update_norm**2 - radial_update**2))
    angular_step = (
        math.atan2(tangential_update, parameter_norm + radial_update)
        if parameter_norm > 0.0
        else 0.0
    )
    matrix_angles: list[float] = accumulator["matrix_angles"]

    return {
        "parameter_norm": parameter_norm,
        "post_parameter_norm": post_parameter_norm,
        "parameter_rms": parameter_norm / math.sqrt(element_count),
        "post_parameter_rms": post_parameter_norm / math.sqrt(element_count),
        "update_norm": update_norm,
        "update_rms": update_norm / math.sqrt(element_count),
        "update_to_parameter_norm": (
            update_norm / parameter_norm if parameter_norm > 0.0 else 0.0
        ),
        "update_weight_cosine": update_weight_cosine,
        "radial_update_fraction": abs(update_weight_cosine),
        "angular_step": angular_step,
        "matrix_angular_step_mean": (
            sum(matrix_angles) / len(matrix_angles) if matrix_angles else 0.0
        ),
        "matrix_angular_step_max": max(matrix_angles, default=0.0),
    }


@torch.no_grad()
def optimizer_update_stats(
    named_parameters: list[tuple[str, torch.nn.Parameter]],
    before_parameters: dict[str, torch.Tensor],
    optimizer_by_parameter_name: dict[str, str],
) -> dict[str, dict[str, float]]:
    """Measure exact AdamW and Muon parameter deltas with one device transfer."""

    accumulators: dict[str, dict[str, Any]] = {
        "all": empty_update_accumulator(),
        "adamw": empty_update_accumulator(),
        "muon": empty_update_accumulator(),
    }
    for name, parameter in named_parameters:
        optimizer_name = optimizer_by_parameter_name[name]
        keys = ["all", optimizer_name]
        if optimizer_name == "muon":
            keys.append(f"muon/{gradient_group_name(name)}")

        before = before_parameters[name]
        after = parameter.detach().to(device="cpu", dtype=torch.float32)
        update = after - before
        for key in keys:
            accumulator = accumulators.setdefault(key, empty_update_accumulator())
            add_parameter_update_to_accumulator(
                accumulator,
                before=before,
                after=after,
                update=update,
    ...
    return {
        group_name: finalize_update_accumulator(accumulator)
        for group_name, accumulator in accumulators.items()
    }


def adjusted_muon_learning_rates(
    optimizer: torch.optim.Muon,
) -> list[float]:
    """Return the effective post-shape-adjustment LR for each Muon matrix."""

    learning_rates: list[float] = []
    for group in optimizer.param_groups:
        for parameter in group["params"]:
            adjustment = muon_learning_rate_adjustment(
                parameter.shape,
                group["adjust_lr_fn"],
            )
            learning_rates.append(group["lr"] * adjustment)
    return learning_rates


def muon_learning_rate_adjustment(
...
def main() -> None:
    ...
    optimizer_muon_parameter_groups = build_muon_parameter_groups(model)
    validate_optimizer_parameter_partition(
        model,
        optimizer_adam_parameter_groups,
    ...
    adamw_decayed_parameter_count = sum(
        parameter.numel() for parameter in optimizer_adam_parameter_groups[0]["params"]
    )
    adamw_no_decay_parameter_count = sum(
        parameter.numel() for parameter in optimizer_adam_parameter_groups[1]["params"]
    )
    adamw_norm_parameter_count = sum(
        parameter.numel() for parameter in optimizer_adam_parameter_groups[2]["params"]
    )
    adamw_parameter_count = (
        adamw_decayed_parameter_count
        + adamw_no_decay_parameter_count
        + adamw_norm_parameter_count
    )
    muon_parameter_count = sum(
        parameter.numel() for parameter in optimizer_muon_parameter_groups[0]["params"]
    )
    optimized_parameter_count = adamw_parameter_count + muon_parameter_count
    if optimized_parameter_count != trainable_parameter_count:
        raise RuntimeError(
            f"optimizers cover {optimized_parameter_count:,} parameters, but the "
            f"model has {trainable_parameter_count:,} trainable parameters"
        )
    ...
    muon_parameter_ids = set(parameter_ids(optimizer_muon_parameter_groups))
    optimizer_by_parameter_name = {
        name: "muon" if id(parameter) in muon_parameter_ids else "adamw"
        for name, parameter in named_parameters
    }
    muon_lr_adjustments = [
        muon_learning_rate_adjustment(parameter.shape, MUON_ADJUST_LR_FN)
        for parameter in optimizer_muon_parameter_groups[0]["params"]
    ]
    muon_adjusted_peak_learning_rate_min = (
        LR * MUON_LR_MULTIPLIER * min(muon_lr_adjustments)
    )
    ...
    settings = {
        ...
        "optimized_parameters": optimized_parameter_count,
        "initialization": INITIALIZATION,
        "initialization_std": INITIALIZATION_STD,
        "qk_norm": QK_NORM,
        ...
        "weight_decay": WEIGHT_DECAY,
        "z_loss_coefficient": Z_LOSS_COEFFICIENT,
        "adamw_parameters": adamw_parameter_count,
        "adamw_decayed_parameters": adamw_decayed_parameter_count,
        "adamw_no_decay_parameters": adamw_no_decay_parameter_count,
        "adamw_norm_parameters": adamw_norm_parameter_count,
        "muon_parameters": muon_parameter_count,
        "muon_parameter_fraction": muon_parameter_count
        / max(trainable_parameter_count, 1),
        ...
        for epoch in range(EPOCHS):
            ...
            for _ in range(optimizer_steps_per_epoch):
                ...
                if should_log_diagnostics:
                    ...
                    before_parameters = snapshot_parameters(named_parameters)
                ...
                learning_rate = optimizer_adam.param_groups[0]["lr"]
                norm_learning_rate = optimizer_adam.param_groups[2]["lr"]
                muon_learning_rate = optimizer_muon.param_groups[0]["lr"]
                muon_adjusted_learning_rates = adjusted_muon_learning_rates(
                    optimizer_muon
                )
                optimizer_adam.step()
                optimizer_muon.step()
                if should_log_diagnostics:
                    update_stats = optimizer_update_stats(
                        named_parameters,
                        before_parameters,
                        optimizer_by_parameter_name,
                    )
                    del before_parameters
                    all_update_stats = update_stats["all"]
                    adamw_update_stats = update_stats["adamw"]
                    muon_update_stats = update_stats["muon"]
                    parameter_norm = all_update_stats["post_parameter_norm"]
                    parameter_rms = all_update_stats["post_parameter_rms"]
                    embedding_norm, embedding_rms, _ = tensor_collection_stats(
                        [model.embeddings.weight]
                    )
                    momentum_buffer_norm, momentum_buffer_rms, _ = (
                        tensor_collection_stats(
                            [
                                optimizer_muon.state[parameter]["momentum_buffer"]
                                for parameter in optimizer_muon_parameter_groups[0][
                                    "params"
                                ]
                                if "momentum_buffer" in optimizer_muon.state[parameter]
                            ]
                        )
                    )
                scheduler_adam.step()
                ...
                writer.add_scalar(
                    "train/gradient_rms",
                    gradient_norm / math.sqrt(trainable_parameter_count),
                    global_step,
                )
                writer.add_scalar(
                ...
                writer.add_scalar(
                    "train/muon_learning_rate", muon_learning_rate, global_step
                )
                writer.add_scalar(
                    "train/muon_adjusted_learning_rate_min",
                    min(muon_adjusted_learning_rates),
                    global_step,
                )
                writer.add_scalar(
                    "train/muon_adjusted_learning_rate_max",
                    max(muon_adjusted_learning_rates),
                    global_step,
                )
                writer.add_scalar(
                    "train/muon_weight_decay_step",
                    muon_learning_rate * MUON_WEIGHT_DECAY,
                    global_step,
                )
                ...
                if should_log_diagnostics:
                    ...
                    writer.add_scalar(
                        ...
                        all_update_stats["update_norm"],
                        global_step,
                    )
                    writer.add_scalar(
                        "diagnostics/update_rms",
                        all_update_stats["update_rms"],
                        global_step,
                    )
                    writer.add_scalar(
                        "diagnostics/update_to_parameter_norm",
                        all_update_stats["update_to_parameter_norm"],
                        global_step,
                    )
                    log_update_stats(
                        writer,
                    ...
                    writer.add_scalar(
                        "optimizer/muon/momentum_buffer_norm",
                        momentum_buffer_norm,
                        global_step,
                    )
                    writer.add_scalar(
                        "optimizer/muon/momentum_buffer_rms",
                        momentum_buffer_rms,
                        global_step,
                    )
                    for update_group_name, group_update_stats in update_stats.items():
                        if not update_group_name.startswith("muon/"):
                            continue
                        log_update_stats(
                            writer,
                            "optimizer/muon_groups/"
                            f"{update_group_name.removeprefix('muon/')}",
                            group_update_stats,
                            global_step,
                        )
                    writer.add_scalar(
                ...
                print(
                    ...
                    + (
                        f" z_loss={train_z_loss:.6f} objective={train_objective:.6f}"
                        if Z_LOSS_COEFFICIENT > 0.0
                        else ""
                    ),
                    flush=True,
