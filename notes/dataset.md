...
uv run python dataset/build_dataset.py \
  --documents-only \
  --train-sizes 10m:10000000,100m:100000000,1b:1000000000 \
...
If a remote stream fails, rerun the same materialization command with
`--resume` instead of `--overwrite`. Completed source checkpoints are reused;
the interrupted source is rebuilt from its start.
...
uv run python src/tokenizer/train_tokenizer.py \
  ...
  --model-prefix src/tokenizer/production_16k \
...
This writes `train_web_{10m,100m,1b}`,
`train_balanced_{10m,100m,1b}`, and
`train_knowledge_{10m,100m,1b}` under `dataset/ds/`, along with one shared
`validation` and `test` artifact. Each size has the stated mixture weights;
smaller artifacts are not token prefixes of the larger ones. To train with a
particular artifact, use a variant such as `DATASET_VARIANT=balanced_100m`.
