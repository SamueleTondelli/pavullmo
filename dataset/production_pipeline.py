...
SIZE_LABEL_RE = re.compile(r"^[a-z0-9][a-z0-9_-]*$")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--train-sizes",
        help=(
            "comma-separated LABEL:TOKENS pairs, e.g. "
            "10m:10000000,100m:100000000,1b:1000000000; "
            "builds each size for every selected mixture"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--resume",
        action="store_true",
        help="resume an interrupted --documents-only build from completed source checkpoints",
    )
...
def source_rows(source: SourceSpec, seed: int, buffer_size: int) -> Iterable[dict]:
    if source.key == "books":
        # Italian-PD shards contain inconsistent bibliographic types and some
        # carry an extra pandas index column. Reading only the two fields we
        # actually use avoids datasets' cross-shard schema cast failure.
        return italian_pd_rows(source, seed, min(buffer_size, source.shuffle_buffer_cap))
...
def italian_pd_rows(source: SourceSpec, seed: int, buffer_size: int) -> Iterator[dict]:
    api = HfApi()
    files = [
        path
        ...
        if re.fullmatch(r"italian_pd_\d+\.parquet", Path(path).name)
    ]
    if not files:
        raise FileNotFoundError(f"no Italian-PD Parquet shards in {source.dataset}")
    rng = random.Random(seed)
    rng.shuffle(files)
    fs = HfFileSystem()
    buffer: list[dict] = []
    for relative_path in files:
        path = f"datasets/{source.dataset}@{source.revision}/{relative_path}"
        with fs.open(path, "rb") as file:
            parquet = pq.ParquetFile(file)
            for batch in parquet.iter_batches(
                batch_size=8, columns=["identifier", "text"]
            ):
                identifiers = batch.column("identifier").to_pylist()
                texts = batch.column("text").to_pylist()
                for identifier, text in zip(identifiers, texts, strict=True):
                    buffer.append({"identifier": identifier, "text": text})
    ...
    *,
    ...
    if temporary_dir.exists():
        if resume:
            ...
            raise FileExistsError(
                f"{temporary_dir} already exists; pass --resume or --overwrite"
            )
        else:
    ...
    for partition, targets in byte_targets.items():
        ...
        for index, (key, target_bytes) in enumerate(targets.items()):
            source_dir = temporary_dir / partition / key
            checkpoint = source_dir / ".complete.json"
            if checkpoint.is_file():
                entry = json.loads(checkpoint.read_text(encoding="utf-8"))
                if entry["target_text_bytes"] != target_bytes:
                    raise ValueError(f"resume target changed for {partition}/{key}")
                manifest_sources[partition][key] = entry
                print(f"Reusing completed {partition}/{key}")
                continue
            if source_dir.exists():
                shutil.rmtree(source_dir)
            counters: Counter[str] = Counter()
            writer = MaterializedParquetWriter(source_dir, shard_bytes)
            hashes: set[str] = set()
...
def parse_train_sizes(value: str | None, default_tokens: int) -> dict[str, int]:
    if value is None:
        return {"": default_tokens}
    sizes: dict[str, int] = {}
    for item in value.split(","):
        label, separator, count = item.strip().partition(":")
        if not separator or not SIZE_LABEL_RE.fullmatch(label):
            raise ValueError(f"invalid --train-sizes entry: {item!r}")
        if label in sizes:
            raise ValueError(f"duplicate --train-sizes label: {label!r}")
        try:
            tokens = int(count)
        except ValueError as error:
            raise ValueError(f"invalid token count in --train-sizes: {item!r}") from error
        if tokens <= 0:
            raise ValueError(f"token count must be positive: {item!r}")
        sizes[label] = tokens
    if not sizes:
    ...
    builder.finish(
        {
            **evaluation_metadata,
            "dataset": {
                ...
                "split": partition,
                "mixture": (
                    mixture_name or name.removeprefix("train_")
                    if partition == "train"
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.resume and not args.documents_only:
        raise ValueError("--resume requires --documents-only")
...
def main() -> None:
    ...
    for mix_name, weights in selected.items():
        for label, tokens in train_sizes.items():
            build_artifact(
                name=f"train_{mix_name}_{label}" if label else f"train_{mix_name}",
                weights=weights,
                total_tokens=tokens,
                partition="train",
